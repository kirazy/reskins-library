...
function reskins.lib.shrink_icon(name, source, scale, inputs)
    -- Initialize paths
    ...
    -- Rescale the icon layers
    for n = 2, #inputs.icon do
        -- Add layer-based icon sizes
        inputs.icon[n].icon_size = inputs.icon_size
        inputs.icon[n].icon_mipmaps = inputs.icon_mipmaps

        -- Scale if it's not a tier-label layer
        if string.find(inputs.icon[n].icon, "tier") then
            inputs.icon[n].scale = 1
        else
            inputs.icon[n].scale = scale
        end
    end

    -- Rescale the picture layers
    for n = 1, #inputs.icon_picture.layers do
        inputs.icon_picture.layers[n].scale = inputs.icon_picture.layers[n].scale*scale
    end
